...
class Login(object):
    ...
    def au_to(self):
        ...
        print(r.status_code)
        for line in r.text.split('\n'): #把登陆页面，网页源代码一行一行打印出来，一行一行遍历
            if 'authenticity_token' in line: #找到有authenticity_token的这一行代码line

                line_new = line.split('value=')[1].split('"')[1]
                #把line进行分隔，以'value='进行分割（跟以勾号进行分割一个道理），去第2个元素（0是第一个
                #元素），同理再进行切分。
        return line_new #返回authenticity_token的密钥

    def email_code(self): #若需要登陆验证，补充即可
    ...
    def login(self, login, password):
        ...
        fp.close()
        response = self.session.get(self.logined_url, headers=self.headers)#Github setting网页
        print(response.status_code)
